...
const AIChatbot = ({ lessonTitle, lessonContent: _lessonContent, currentQuestion: _currentQuestion }: AIChatbotProps) => {
  const currentTopic =
    localStorage.getItem("selectedTopicTitle") || lessonTitle || "this lesson";
  const [messages, setMessages] = useState<Message[]>([
    {
      type: "bot",
      text: `Hi there! I'm your AI tutor. Ask me anything about ${currentTopic}. If it's not about this lesson, I'll ask you to stay on topic.`,
    },
  ]);
  ...
  const handleSendMessage = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const responseText = await askAITutor(currentTopic, question);
  ...
  return (
    <div className="relative group overflow-hidden rounded-xl border border-white/10 bg-zinc-900/50 backdrop-blur-sm transition-all duration-300 hover:border-white/30 hover:bg-zinc-900/70 h-[520px] min-h-[520px] max-h-[520px] flex flex-col">
