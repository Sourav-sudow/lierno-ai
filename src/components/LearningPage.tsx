...
import { generateMCQsFromTopic, GeneratedMCQ } from "../services/openRouterQuiz";
...
const LearningPage = () => {
  ...
  const selectedTopicTitle = localStorage.getItem("selectedTopicTitle") || "";
  const selectedTopicVideoUrl = localStorage.getItem("selectedTopicVideoUrl") || "";
  const selectedTopicNarration = localStorage.getItem("selectedTopicNarration") || "";
  ...
  const [quizItems, setQuizItems] = useState<GeneratedMCQ[]>([]);
  ...
  const displayTotal = quizItems.length
    ? Math.min(totalQuizzes, quizItems.length)
    : totalQuizzes;

  const currentQuestion = quizItems[currentQuizIndex];
  const question = currentQuestion?.question || "Generating quiz...";
  const answer = currentQuestion?.choices || [];
  const correctAnswerIndex = currentQuestion?.correctIndex ?? 0;
  function handleAnswerClick(currIndex: number) {
    if (!currentQuestion) return;
    setSelectedAnswerIndex(currIndex);
    if (!hasAnswered) {
      setCompletedQuizzes((prev) => Math.min(displayTotal, prev + 1));
    ...
    setHasAnswered(true);
    // Auto-advance to next quiz after short delay
    setTimeout(() => {
      setSelectedAnswerIndex(null);
      setHasAnswered(false);
      setCurrentQuizIndex((prev) => {
        const next = prev + 1;
        if (next < quizItems.length && next < totalQuizzes) return next;
        return prev; // stop at last available
      });
    }, 1200);
  }
  ...
  const displayTitle = selectedTopicTitle || currentSlide?.title || "";
  const displayNarration = selectedTopicNarration || currentSlide?.narration || "";
  ...
  useEffect(() => {
    const loadQuiz = async () => {
      ...
      setQuizError(null);
      try {
        const items = await generateMCQsFromTopic(
          displayTitle || "SQL",
          displayNarration || "Use the lesson transcript to craft SQL MCQs",
          totalQuizzes
        );
        setQuizItems(items);
        ...
        >
          ...
          <div className="relative group overflow-hidden rounded-xl border border-white/10 md:col-span-2 h-72 md:h-96 bg-zinc-900/50 backdrop-blur-sm transition-all duration-300 hover:border-white/30 hover:bg-zinc-900/70">
            ...
            <div className="h-full w-full flex flex-col gap-3 items-center justify-center p-2 text-center">
              {selectedTopicTitle ? (
                <h3 className="text-white font-semibold text-lg">{selectedTopicTitle}</h3>
              ) : null}
          ...
          <div className="relative group overflow-hidden rounded-xl border border-white/10 p-4 min-h-[18rem] h-auto bg-zinc-900/50 backdrop-blur-sm transition-all duration-300 hover:border-white/30 hover:bg-zinc-900/70">
            ...
            <div className="relative z-10 flex flex-col h-full">
              ...
              <p className="text-white/70 mb-4">
                {quizLoading ? "Generating quiz..." : quizError ? quizError : question}
              </p>
              <div className="space-y-3 flex-grow">
                {answer.map((info: string, index: number) => (
                  <motion.button
                    ...
                    key={index}
                    disabled={hasAnswered || quizLoading || !!quizError || !currentQuestion}
                  >
                    {info}
                  </motion.button>
                ))}
          ...
          <div className="relative group overflow-hidden rounded-xl border border-white/10 p-6 bg-zinc-900/50 backdrop-blur-sm transition-all duration-300 hover:border-white/30 hover:bg-zinc-900/70">
            ...
            <div className="absolute inset-0 bg-grid-white/[0.02] bg-[size:20px_20px]"></div>
                  <div className="h-full w-full flex flex-col justify-center gap-4 relative z-10">
                    <div>
                      <p className="text-white font-semibold text-lg mb-1">
                        Quiz Progress: {completedQuizzes} / {displayTotal} Completed
                      </p>
                      ...
                      <div className="w-full h-3 rounded-full bg-white/10 overflow-hidden">
                        <div
                          ...
                          style={{ width: `${Math.min(100, (completedQuizzes / displayTotal) * 100)}%` }}
                        ></div>
                      </div>
                    </div>
                    <div className="flex items-center">
                      ...
                      >
                        <span>{hasAnswered ? "Continue Quiz" : "Resume Quiz"}</span>
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
                          viewBox="0 0 24 24"
